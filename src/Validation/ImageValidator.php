    ...
    public function validate($value)
    {
        ...
        if (!empty($filePath) && is_file($filePath)) {
            ...
            if ($this->maxWidth !== null && $width > $this->maxWidth) {
                $this->addError(app()->t('validation', "Maximum image width: {size}", [
                    '{size}' => $this->maxWidth,
                ]));
            }
            if ($this->maxHeight !== null && $height > $this->maxHeight) {
                $this->addError(app()->t('validation', "Maximum image height: {size}", [
                    '{size}' => $this->maxHeight,
                ]));
            }
            if ($this->minWidth !== null && $width < $this->minWidth) {
                $this->addError(app()->t('validation', "Minimum image width: {size}", [
                    '{size}' => $this->minWidth,
                ]));
            }
